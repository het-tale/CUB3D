...
/*   Updated: 2023/01/03 09:05:06 by het-tale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_horz_ray(t_mlx *mlx, t_ray *ray, t_raycast *rcst)
{
	...
	if (ray->is_ray_facing_left && rcst->xstep > 0)
		rcst->xstep *= -1;
	else if (ray->is_ray_facing_right && rcst->xstep < 0)
		rcst->xstep *= -1;
...
void	cast_vert_ray(t_mlx *mlx, t_ray *ray, t_raycast *rcst)
{
	...
	if (ray->is_ray_facing_up && rcst->ystep > 0)
		rcst->ystep *= -1;
	else if (ray->is_ray_facing_down && rcst->ystep < 0)
		rcst->ystep *= -1;
...
void	cast_ray(t_mlx *mlx, t_ray *ray)
{
	...
	if (rcst->horz_hit_distance < rcst->vert_hit_distance)
	{
		...
		ray->wall_hit_x = rcst->horz_wall_hit_x;
		ray->wall_hit_y = rcst->horz_wall_hit_y;
		ray->was_hit_vertical = 0;
	}
	else
	{
		ray->distance = rcst->vert_hit_distance;
		ray->wall_hit_x = rcst->vert_wall_hit_x;
		ray->wall_hit_y = rcst->vert_wall_hit_y;
